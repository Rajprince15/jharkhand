...
async def get_handicrafts(
    ...
    """Get all handicrafts with optional filtering and search"""
    try:
        pool = await get_db()
        async with pool.acquire() as conn:
            async with conn.cursor(aiomysql.DictCursor) as cursor:
                ...
                if location:
                    where_conditions.append("h.origin_village LIKE %s")
                ...
                if min_rating:
                    where_conditions.append("h.rating >= %s")
                    params.append(min_rating)
                
                if is_featured is not None:
                    where_conditions.append("h.is_featured = %s")
                    params.append(is_featured)
                
                if search:
                    where_conditions.append("(h.name LIKE %s OR h.description LIKE %s OR h.materials LIKE %s)")
                    params.extend([f"%{search}%", f"%{search}%", f"%{search}%"])
                
                where_clause = " AND ".join(where_conditions)
                
                # Valid sort columns
                valid_sort_columns = ['created_at', 'price', 'rating', 'name']
                ...
                query = f"""
                    SELECT 
                        ...
                        COALESCE(h.images, '[]') as images,
                        COALESCE(h.tags, '[]') as tags
                    FROM handicrafts h
                    LEFT JOIN users u ON h.seller_id = u.id
                    WHERE {where_clause}
                    ORDER BY h.{sort_column} {order}
                    LIMIT %s OFFSET %s
                """
                
                params.extend([limit, offset])
                await cursor.execute(query, params)
                handicrafts = await cursor.fetchall()
                
                # Get total count
                count_query = f"SELECT COUNT(*) as total FROM handicrafts h WHERE {where_clause}"
                await cursor.execute(count_query, params[:-2])  # Remove limit and offset
                total_result = await cursor.fetchone()
                total = total_result['total'] if total_result else 0
                
                # Parse JSON fields
                for item in handicrafts:
                    if item['images']:
                        try:
...
@api_router.get("/marketplace/handicrafts/{handicraft_id}")
async def get_handicraft(handicraft_id: str):
    """Get specific handicraft by ID"""
    try:
        pool = await get_db()
        async with pool.acquire() as conn:
            async with conn.cursor(aiomysql.DictCursor) as cursor:
                query = """
                    ...
                    FROM handicrafts h
                    LEFT JOIN users u ON h.seller_id = u.id
                    WHERE h.id = %s AND h.is_available = 1
                """
                
                await cursor.execute(query, (handicraft_id,))
                handicraft = await cursor.fetchone()
                
                if not handicraft:
                    raise HTTPException(status_code=404, detail="Handicraft not found")
                
                # Parse JSON fields
                if handicraft['images']:
                    try:
                        handicraft['images'] = json.loads(handicraft['images'])
                    except:
                        handicraft['images'] = []
                else:
                    handicraft['images'] = []
                ...
                if handicraft['tags']:
                    try:
                        handicraft['tags'] = json.loads(handicraft['tags'])
                    except:
                        handicraft['tags'] = []
                else:
                    handicraft['tags'] = []
...
@api_router.post("/marketplace/handicrafts")
async def create_handicraft(handicraft_data: HandicraftCreate, current_user: dict = Depends(get_current_user)):
    """Create a new handicraft (for artisans/providers)"""
    try:
        # Only providers can create handicrafts
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cursor:
                handicraft_id = f"hc_{uuid.uuid4().hex[:12]}"
                
                # Convert lists to JSON
                images_json = json.dumps(handicraft_data.images or [])
                tags_json = json.dumps(handicraft_data.tags or [])
                ...
                query = """
                    INSERT INTO handicrafts (
                        ...
                        cultural_significance, care_instructions, tags
                    ) VALUES (
                ...
                await cursor.execute(query, (
                    ...
                    handicraft_data.category.value, handicraft_data.description,
                    handicraft_data.price, handicraft_data.discount_price,
                    handicraft_data.stock_quantity, images_json, handicraft_data.materials,
                    handicraft_data.dimensions, handicraft_data.weight, handicraft_data.origin_village,
                    handicraft_data.cultural_significance, handicraft_data.care_instructions, tags_json
                ))
...
@api_router.put("/marketplace/handicrafts/{handicraft_id}")
async def update_handicraft(handicraft_id: str, handicraft_data: HandicraftUpdate, current_user: dict = Depends(get_current_user)):
    """Update handicraft (only by owner or admin)"""
    try:
        pool = await get_db()
        async with pool.acquire() as conn:
            async with conn.cursor() as cursor:
                # Check ownership
                await cursor.execute("SELECT seller_id FROM handicrafts WHERE id = %s", (handicraft_id,))
                handicraft = await cursor.fetchone()
                
                if not handicraft:
                    raise HTTPException(status_code=404, detail="Handicraft not found")
                
                if current_user['role'] != 'admin' and handicraft[0] != current_user['id']:
                    raise HTTPException(status_code=403, detail="Not authorized to update this handicraft")
                
                # Build update query dynamically
                update_fields = []
                params = []
                
                for field, value in handicraft_data.dict(exclude_unset=True).items():
                ...
                query = f"UPDATE handicrafts SET {', '.join(update_fields)} WHERE id = %s"
                await cursor.execute(query, params)
                
                return {"message": "Handicraft updated successfully"}
...
# Handicraft Orders Management
@api_router.post("/marketplace/handicrafts/{handicraft_id}/order")
async def create_handicraft_order(handicraft_id: str, order_data: HandicraftOrderCreate, current_user: dict = Depends(get_current_user)):
    """Place an order for a handicraft"""
    try:
        pool = await get_db()
        async with pool.acquire() as conn:
            async with conn.cursor(aiomysql.DictCursor) as cursor:
                # Get handicraft details
                await cursor.execute("""
                    SELECT id, seller_id, name, price, stock_quantity, is_available 
                    FROM handicrafts WHERE id = %s
                """, (handicraft_id,))
                handicraft = await cursor.fetchone()
                ...
                if not handicraft['is_available']:
                    raise HTTPException(status_code=400, detail="Handicraft is not available")
                
                if handicraft['stock_quantity'] < order_data.quantity:
                    raise HTTPException(status_code=400, detail="Insufficient stock")
                
                if handicraft['seller_id'] == current_user['id']:
                    raise HTTPException(status_code=400, detail="Cannot order your own handicraft")
                
                # Create order
                order_id = f"ho_{uuid.uuid4().hex[:12]}"
                unit_price = handicraft['price']
                total_price = unit_price * order_data.quantity
                shipping_address_json = json.dumps(order_data.shipping_address.dict())
                
                insert_query = """
                    INSERT INTO handicraft_orders (
                        id, user_id, handicraft_id, seller_id, quantity, unit_price, 
                        total_price, shipping_address, shipping_method, buyer_notes
                    ) VALUES (
                        %s, %s, %s, %s, %s, %s, %s, %s, %s, %s
                    )
                """
                ...
                await cursor.execute(insert_query, (
                    order_id, current_user['id'], handicraft_id, handicraft['seller_id'],
                    order_data.quantity, unit_price, total_price, shipping_address_json,
                    order_data.shipping_method, order_data.buyer_notes
                ))
                
                # Update stock
                await cursor.execute(
                    "UPDATE handicrafts SET stock_quantity = stock_quantity - %s WHERE id = %s",
                    (order_data.quantity, handicraft_id)
                )
        ...
        async with pool.acquire() as conn:
            async with conn.cursor(aiomysql.DictCursor) as cursor:
                query = """
                    SELECT 
                        o.*,
                        h.name as handicraft_name,
                        h.images as handicraft_images,
                        u.name as seller_name
                    FROM handicraft_orders o
                    LEFT JOIN handicrafts h ON o.handicraft_id = h.id
                ...
                for order in orders:
                    ...
                    if order['handicraft_images']:
                        try:
                            order['handicraft_images'] = json.loads(order['handicraft_images'])
                        except:
                            order['handicraft_images'] = []
                    else:
                        order['handicraft_images'] = []
...
async def get_cultural_events(
    ...
    """Get cultural events with filtering"""
    try:
        pool = await get_db()
        async with pool.acquire() as conn:
            async with conn.cursor(aiomysql.DictCursor) as cursor:
                where_conditions = ["e.is_active = 1", "e.end_date >= NOW()"]
                params = []
                ...
                if min_price:
                    where_conditions.append("e.price >= %s")
                    params.append(min_price)
                ...
                if start_date:
                    where_conditions.append("DATE(e.start_date) >= %s")
                    params.append(start_date)
                
                if is_featured is not None:
                    where_conditions.append("e.is_featured = %s")
                    params.append(is_featured)
                ...
                query = f"""
                    SELECT 
                        ...
                        COALESCE(e.contact_info, '{{}}') as contact_info
                ...
                for event in events:
                    ...
                    if event['contact_info']:
                        try:
                            event['contact_info'] = json.loads(event['contact_info'])
                        except:
                            event['contact_info'] = {}
                    else:
                        event['contact_info'] = {}
                ...
                # Get total count
                count_query = f"SELECT COUNT(*) as total FROM cultural_events e WHERE {where_clause}"
...
@api_router.post("/marketplace/events")
async def create_cultural_event(event_data: CulturalEventCreate, current_user: dict = Depends(get_current_user)):
    """Create a new cultural event (for providers/organizers)"""
    try:
        if current_user['role'] not in ['provider', 'admin']:
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cursor:
                ...
                images_json = json.dumps(event_data.images or [])
                tags_json = json.dumps(event_data.tags or [])
                contact_info_json = json.dumps(event_data.contact_info or {})
                ...
                await cursor.execute(query, (
                    ...
                    event_data.event_type.value, event_data.location, event_data.venue_details,
                    event_data.start_date, event_data.end_date, event_data.price,
                    event_data.max_participants, images_json, event_data.cultural_significance,
                    event_data.what_to_expect, event_data.what_to_bring, event_data.age_restrictions,
                    event_data.languages, contact_info_json, event_data.cancellation_policy, tags_json
                ))
...
# Homestays Management (extending existing providers system)
@api_router.get("/marketplace/homestays")
async def get_homestays(
    ...
    """Get homestays (accommodation providers with extended features)"""
    try:
        pool = await get_db()
        async with pool.acquire() as conn:
            async with conn.cursor(aiomysql.DictCursor) as cursor:
                where_conditions = ["p.category = 'accommodation'", "p.is_active = 1"]
                params = []
                
                if location:
                    where_conditions.append("p.location LIKE %s")
                    params.append(f"%{location}%")
                ...
                if max_guests:
                    where_conditions.append("p.max_guests >= %s")
                    params.append(max_guests)
                
                if destination_id:
                    where_conditions.append("p.destination_id = %s")
                    params.append(destination_id)
                
                if search:
                    where_conditions.append("(p.name LIKE %s OR p.service_name LIKE %s OR p.description LIKE %s)")
                    params.extend([f"%{search}%", f"%{search}%", f"%{search}%"])
                ...
                query = f"""
                    SELECT 
                        ...
                        u.name as host_name,
                        d.name as destination_name,
                        COALESCE(p.amenities, '[]') as amenities,
                        COALESCE(p.cultural_activities, '[]') as cultural_activities,
                        COALESCE(p.meal_options, '[]') as meal_options
                    FROM providers p
                ...
                params.extend([limit, offset])
                await cursor.execute(query, params)
                homestays = await cursor.fetchall()
                
                # Parse JSON fields
                for homestay in homestays:
                    for json_field in ['amenities', 'cultural_activities', 'meal_options']:
                        if homestay[json_field]:
                            try:
                                homestay[json_field] = json.loads(homestay[json_field])
                            except:
                                homestay[json_field] = []
                        else:
                            homestay[json_field] = []
                ...
                # Get total count
                count_query = f"SELECT COUNT(*) as total FROM providers p WHERE {where_clause}"
                await cursor.execute(count_query, params[:-2])
                total_result = await cursor.fetchone()
                total = total_result['total'] if total_result else 0
...
@api_router.post("/marketplace/homestays")
async def create_homestay(homestay_data: HomestayCreate, current_user: dict = Depends(get_current_user)):
    """Create a new homestay (extends provider system)"""
    try:
        if current_user['role'] not in ['provider', 'admin']:
            raise HTTPException(status_code=403, detail="Only providers can create homestays")
        
        pool = await get_db()
        async with pool.acquire() as conn:
            async with conn.cursor() as cursor:
                homestay_id = f"hs_{uuid.uuid4().hex[:12]}"
                
                # Convert lists to JSON
                amenities_json = json.dumps(homestay_data.amenities or [])
                cultural_activities_json = json.dumps(homestay_data.cultural_activities or [])
                meal_options_json = json.dumps(homestay_data.meal_options or [])
                
                query = """
                    INSERT INTO providers (
                        id, user_id, name, category, service_name, description, price, location,
                        contact, destination_id, image_url, amenities, house_rules, max_guests,
                        rooms_available, cultural_activities, meal_options, family_type
                    ) VALUES (
                        %s, %s, %s, 'accommodation', %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s
                    )
                """
                
                await cursor.execute(query, (
                    homestay_id, current_user['id'], homestay_data.name, homestay_data.service_name,
                    homestay_data.description, homestay_data.price, homestay_data.location,
                    homestay_data.contact, homestay_data.destination_id, homestay_data.image_url,
                    amenities_json, homestay_data.house_rules, homestay_data.max_guests,
                    homestay_data.rooms_available, cultural_activities_json, meal_options_json,
                    homestay_data.family_type
                ))
...
# Marketplace Reviews
@api_router.post("/marketplace/handicrafts/{handicraft_id}/reviews")
async def create_handicraft_review(handicraft_id: str, review_data: HandicraftReviewCreate, current_user: dict = Depends(get_current_user)):
    """Create a review for a handicraft (must have purchased)"""
    try:
        pool = await get_db()
        async with pool.acquire() as conn:
            async with conn.cursor(aiomysql.DictCursor) as cursor:
                # Verify the user has ordered this handicraft
                await cursor.execute("""
                    SELECT id FROM handicraft_orders 
                    WHERE id = %s AND user_id = %s AND handicraft_id = %s AND status = 'delivered'
                """, (review_data.order_id, current_user['id'], handicraft_id))
                
                order = await cursor.fetchone()
                if not order:
                    raise HTTPException(status_code=400, detail="You can only review handicrafts you have purchased and received")
                
                # Check if review already exists
                await cursor.execute("""
                    SELECT id FROM handicraft_reviews 
                    WHERE user_id = %s AND handicraft_id = %s AND order_id = %s
                """, (current_user['id'], handicraft_id, review_data.order_id))
                
                existing_review = await cursor.fetchone()
                if existing_review:
                    raise HTTPException(status_code=400, detail="You have already reviewed this handicraft")
                
                # Create review
                review_id = f"hr_{uuid.uuid4().hex[:12]}"
                images_json = json.dumps(review_data.images or [])
                
                query = """
                    INSERT INTO handicraft_reviews (
                        id, user_id, handicraft_id, order_id, rating, review_text,
                        quality_rating, delivery_rating, seller_rating, images
                    ) VALUES (
                        %s, %s, %s, %s, %s, %s, %s, %s, %s, %s
                    )
                """
                ...
                await cursor.execute(query, (
                    review_id, current_user['id'], handicraft_id, review_data.order_id,
                    review_data.rating, review_data.review_text, review_data.quality_rating,
                    review_data.delivery_rating, review_data.seller_rating, images_json
                ))
                
                # Update handicraft rating
                await cursor.execute("""
                    UPDATE handicrafts SET 
                        rating = (SELECT AVG(rating) FROM handicraft_reviews WHERE handicraft_id = %s),
                        total_reviews = (SELECT COUNT(*) FROM handicraft_reviews WHERE handicraft_id = %s)
                    WHERE id = %s
                """, (handicraft_id, handicraft_id, handicraft_id))
...
@api_router.get("/marketplace/handicrafts/{handicraft_id}/reviews")
async def get_handicraft_reviews(handicraft_id: str, page: int = 1, limit: int = 10):
    """Get reviews for a handicraft"""
    try:
        pool = await get_db()
        async with pool.acquire() as conn:
            async with conn.cursor(aiomysql.DictCursor) as cursor:
                ...
                query = """
                    ...
                    FROM handicraft_reviews r
                    LEFT JOIN users u ON r.user_id = u.id
                    WHERE r.handicraft_id = %s
                    ORDER BY r.created_at DESC
                    LIMIT %s OFFSET %s
                """
                
                await cursor.execute(query, (handicraft_id, limit, offset))
                ...
                # Get total count
                await cursor.execute("SELECT COUNT(*) as total FROM handicraft_reviews WHERE handicraft_id = %s", (handicraft_id,))
                total_result = await cursor.fetchone()
                total = total_result['total'] if total_result else 0
    ...
    try:
        ...
        async with pool.acquire() as conn:
            async with conn.cursor(aiomysql.DictCursor) as cursor:
                ...
                # Total handicrafts
                await cursor.execute("SELECT COUNT(*) as total FROM handicrafts WHERE is_available = 1")
                result = await cursor.fetchone()
                stats['total_handicrafts'] = result['total'] if result else 0
                
                # Total events
                await cursor.execute("SELECT COUNT(*) as total FROM cultural_events WHERE is_active = 1 AND end_date >= NOW()")
                result = await cursor.fetchone()
                stats['total_events'] = result['total'] if result else 0
                
                # Total homestays
                await cursor.execute("SELECT COUNT(*) as total FROM providers WHERE category = 'accommodation' AND is_active = 1")
                result = await cursor.fetchone()
                stats['total_homestays'] = result['total'] if result else 0
                
                # Total artisans (users with handicrafts)
                await cursor.execute("SELECT COUNT(DISTINCT seller_id) as total FROM handicrafts WHERE is_available = 1")
                result = await cursor.fetchone()
                stats['total_artisans'] = result['total'] if result else 0
                
                # Total orders
                await cursor.execute("SELECT COUNT(*) as total FROM handicraft_orders")
                result = await cursor.fetchone()
                ...
                await cursor.execute("""
                    SELECT 
                        (SELECT COUNT(*) FROM handicrafts WHERE is_featured = 1) +
                        (SELECT COUNT(*) FROM cultural_events WHERE is_featured = 1) as total
                """)
                result = await cursor.fetchone()
                stats['featured_items_count'] = result['total'] if result else 0
                
                # Average rating
                await cursor.execute("""
                    SELECT AVG(rating) as avg_rating FROM (
                        SELECT rating FROM handicrafts WHERE rating > 0
                        UNION ALL
                        SELECT rating FROM cultural_events WHERE rating > 0
                        UNION ALL
                        SELECT rating FROM providers WHERE category = 'accommodation' AND rating > 0
                    ) as all_ratings
                """)
                result = await cursor.fetchone()
